...
public class ClinicDao {
    ...
    public void add(Clinic clinic) throws SQLException {
        String sql = "insert into clinic (id, name, specialty" +
                ", address, phone, mobile, email, doctor_name)values(?,?,?,?,?,?,?,?)";
    ...
    public void update(Clinic clinic) throws SQLException {
        String sql = "update clinic set name=?, specialty=?" +
                ", address=?, phone=?, mobile=?, email=?, doctor_name=? where id=?";
        PreparedStatement st = connection.prepareStatement(sql);
